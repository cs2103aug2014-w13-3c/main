		...
		for(unsigned int j = 0; j < optionFieldsChecker.size(); j++){
			...
			// Only recursive option field is supported here
				...
				if(isInsufficientParameters(commandStringTokens)){
					...
					checkRecursiveOptionConstraints(j, cmdParamAndOptMap, Param, cmdType);
				}
				return cmdParamAndOptMap;
...
//@author A0098802X
void Parser::checkRecursiveOptionConstraints(unsigned int j, 
										   multimap<string,any> cmdParamAndOptMap, 
										   string Param, 
										   commandTypeEnum::COMMAND_TYPE cmdType){
...
multimap<string, any> Parser::extractOptionsAndValues(commandTypeEnum::COMMAND_TYPE cmdType,
	...
	if(!hasFieldValueFormat){
		processComplete = processOptionWithoutNeededValues(cmdParamAndOptMap, mmOptionKey, fieldPos, commandStringTokens, validUntilNow, cmdType, processComplete);
				...
				for(unsigned int j = 0; j < optionFieldsChecker.size(); j++){
					...
					if(areEqualStringsIgnoreCase(commandStringTokens[i+1], get<0>(optionFieldsChecker[j]))){
						if(hasFieldValueEntered){
							...
							identifyAndProcessOption(mmOptionKey, fieldValueVector, cmdType, cmdParamAndOptMap, validUntilNow, fieldValue, commandStringTokens, i, j);
						}
...
void Parser::identifyAndProcessOption(string mmOptionKey, 
...
bool Parser::processOptionWithoutNeededValues(multimap<string, any> &cmdParamAndOptMap, 
											  string mmOptionKey, 
											  int fieldPos, 
											  vector<string> &commandStringTokens, 
											  bool validUntilNow, 
